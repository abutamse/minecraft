...
const uiHealth=$("health"),uiHunger=$("hunger"),uiCoins=$("coins");
const hotbar=$("hotbar");
...
const tex={
 ...
 wood:T("wood.png"),leaves:T("leaves.png"),cow:T("cow.png")
};
...
/* ===== Input ===== */
const move={f:0,b:0,l:0,r:0,j:0};
onkeydown=e=>{
 if(e.key=="w")move.f=1;
 if(e.key=="s")move.b=1;
 if(e.key=="a")move.l=1;
...
function addBlock(x,y,z,t){
 ...
 scene.add(m); blocks.push({x,y,z,m});
 map[k]=t;
}
...
/* ===== Inventory + Crafting ===== */
let inventory=JSON.parse(localStorage.inv||"{}");
if(!inventory.grass)inventory={grass:10,dirt:10,wood:5,plank:0};
let selected="grass";
...
function ui(){
 ...
 localStorage.inv=JSON.stringify(inventory);
}

/* ===== Raycast (GENAU FADENKREUZ) ===== */
const ray=new THREE.Raycaster();
function target(add){
 ray.setFromCamera({x:0,y:0},camera);
 const hit=ray.intersectObjects(blocks.map(b=>b.m))[0];
 if(!hit)return null;
...
shootBtn.ontouchstart=()=>{
 const s=new THREE.Mesh(new THREE.SphereGeometry(.1),new THREE.MeshBasicMaterial({color:0xff0000}));
...
const animals=[];
for(let i=0;i<6;i++){
 const a=new THREE.Mesh(new THREE.BoxGeometry(.8,.8,.8),new THREE.MeshLambertMaterial({map:tex.cow}));
 a.position.set(Math.random()*30-15,10,Math.random()*30-15);
 while(collide(a.position))a.position.y++;
 a.hp=10; animals.push(a); scene.add(a);
}
...
function loop(){
 ...
 /* Day/Night */
 dayTime+=dt*.02;
 sun.intensity=.2+Math.max(0,Math.sin(dayTime));
 ambient.intensity=sun.intensity*.6;
 ...
 /* Movement */
 let dx=move.r-move.l,dz=move.b-move.f;
 ...
 player.vel.y-=9.8*dt;
 if(move.j&&player.onGround){player.vel.y=6;player.onGround=false;}
 player.pos.y+=player.vel.y*dt;
 if(collide(player.pos)){player.vel.y=0;player.onGround=true;player.pos.y=Math.ceil(player.pos.y);}
 ...
 for(let i=shots.length-1;i>=0;i--){
  ...
  for(const a of animals){
   if(s.position.distanceTo(a.position)<.5){
    ...
    if(a.hp<=0){scene.remove(a);player.coins+=5;inventory.meat=(inventory.meat||0)+1;ui();}
   }
  }
 }

 uiHealth.textContent="❤️ "+player.hp|0;
 uiHunger.textContent="🍖 "+player.hunger|0+"%";
