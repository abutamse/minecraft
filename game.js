...
function updateStats() {
  healthUI.textContent = `❤️ ${Math.max(0, Math.floor(player.hp))}`;
...
function saveWorld() {
  localStorage.setItem(key("world"), JSON.stringify(world));
}
...
const weapons = [
  ...
  { name: "Schwert III", dmg: 35, price: 700 }
];
...
function spawnMob(type) {
  const mat = new THREE.MeshLambertMaterial({
    color: type === "animal" ? 0x996633 : 0xaa0000
  });
...
function hitMob() {
  ...
  const dmg = weapons.find(w => w.name === currentWeapon).dmg;
  mob.hp -= dmg;
...
/* ---------- BUTTONS ---------- */
jump.onclick = () => {
  if (player.pos.y <= 3.01) player.vel.y = 6;
};

mine.onclick = () => {
  const b = getTargetBlock();
  if (b) removeBlock(b);
  else hitMob();
};
