...
const player = {
  ...
  onGround:false, hp:100, hunger:100, coins:0
};

/* ================= INVENTORY ================= */
const inventory = { grass:0,dirt:0,stone:0,sand:0,wood:0,leaves:0,meat:0 };
let selected = "dirt";
...
$("build").onclick=()=>{ 
  ...
  if(t){ inventory[selected]--; updateHotbar(); addBlock(t.x|0,t.y|0,t.z|0,selected); }
};
...
function loop(){
  ...
  if(collides(player.pos)){ player.vel.y=0; player.pos.y=Math.ceil(player.pos.y); player.onGround=true; }
  ...
  for(let i=bullets.length-1;i>=0;i--){
    ...
    if(t && b.position.distanceTo(new THREE.Vector3(t.x+0.5,t.y+0.5,t.z+0.5))<0.6){
