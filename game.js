...
function initGame() {
    ...
    function target(add) {
        ...
        return add ? { x: p.x - 0.5 + n.x, y: p.y - 0.5 + n.y, z: p.z - 0.5 + n.z }
    ...
    function updateCraftMenu() {
        ...
        woodBtn.onclick = () => {
            if (inventory.wood >= 1) { inventory.wood--; inventory.plank = (inventory.plank || 0) + 4; updateCraftMenu(); updateBuildMenu(); updateHotbarUI(); }
        };
        ...
        plankBtn.onclick = () => {
            if (inventory.plank >= 4) { inventory.plank -= 4; inventory.stone += 1; updateCraftMenu(); updateBuildMenu(); updateHotbarUI(); }
        };
    ...
    function spawnAnimalSafe(x, z){
        ...
        scene.add(m);
        animals.push({ mesh:m, hp:10, dir:new THREE.Vector3(Math.random()-0.5,0,Math.random()-0.5).normalize(), time:2+Math.random()*3 });
    }
    ...
    function animate(){
        ...
        if(collides(player.pos)){ player.vel.y=0; player.onGround=true; player.pos.y=Math.ceil(player.pos.y); }
