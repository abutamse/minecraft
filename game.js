...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function key(k) { return `${k}_${playerName}`; }
    ...
    function updateWeaponsUI() {
      ...
      weapons.forEach(w => {
        ...
        if (ownedWeapons.includes(w.name)) {
          ...
          btn.onclick = () => { if (player.coins < w.price) return; player.coins -= w.price; ownedWeapons.push(w.name); currentWeapon = w.name; localStorage.setItem(key("weapons"), JSON.stringify(ownedWeapons)); localStorage.setItem(key("currentWeapon"), currentWeapon); updateWeaponsUI(); };
        }
